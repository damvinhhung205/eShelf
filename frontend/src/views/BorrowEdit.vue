...
onMounted(async () => {
    try {
        ...
        borrowDate.value = new Date(borrow.value.borrowDate).toISOString().slice(0, 10);
        resetForm({
            values: {
                returnDate: new Date(borrow.value.returnDate).toISOString().slice(0, 10)
            }
        });
    } catch (error) {
        console.log(error);
            ...
            <form @submit.prevent>
                <fieldset class="fieldset bg-base-200 border-base-300 rounded-box w-xs border p-4 text-base">
                    ...
                    <input type="text" class="input" id="user_id" readonly
                        :value=" borrow.userID?.lastName + ' ' + borrow.userID?.firstName " />

                    <label class="label" for="title">Tựa sách</label>
                    <input type="text" class="input" id="title" readonly :value=" borrow.bookID?.title " />

                    <label class="label" for="staff_id">Nhân viên duyệt</label>
                    <input type="text" class="input" id="staff_id" readonly :value=" borrow.staffID?.fullName || 'Không xác định' ">
